...
public interface TransactionRepository extends JpaRepository<Transaction, Long> {
  ...
  @Query("SELECT t FROM Transaction t INNER JOIN t.category c WHERE c.transactionType = 3 AND t.wallet.id = :walletId AND t.isFinished = false")
  List<Transaction> getLoanTransaction (@Param("walletId") Long walletId);
}
